  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        `);
            ...
            chunks.push(ts_poet_1.code `if (object.${jsonName} !== undefined && object.${jsonName} !== null) {`);
      ...
      `);
            ...
            chunks.push(ts_poet_1.code `message.${fieldName} = object.${fieldName};`);
        }
        else if (types_1.isStructType(field)) {
            chunks.push(ts_poet_1.code `message.${fieldName} = typeof(object.${fieldName}) === 'object' ? object.${fieldName} : undefined;`);
        }
        else if (types_1.isListValueType(field)) {
            chunks.push(ts_poet_1.code `
...
function generateWrap(fullProtoTypeName) {
    const chunks = [];
    ...
    if (types_1.isListValueTypeName(fullProtoTypeName)) {
        chunks.push(ts_poet_1.code `wrap(value: Array<any>): ListValue {
      return {values: value};
    }`);
    }
    return chunks;
}
function generateUnwrap(fullProtoTypeName) {
    const chunks = [];
    if (types_1.isStructTypeName(fullProtoTypeName)) {
